    ...
    function refreshCode() {
		...
		if(code == '') {
			...
			jQuery("[id^=" + sc_id + "_]").each(function(index) {
				if((jQuery(this).attr('value') != '' && jQuery(this).attr('type') != 'checkbox') || (jQuery(this).attr('type') == 'checkbox' && (jQuery(this).attr('checked') || jQuery(this).hasClass('forced')))) {
					if(jQuery(this).attr('type') == 'checkbox' && !jQuery(this).attr('checked') && jQuery(this).hasClass('forced'))
						code += ' ' + jQuery(this).attr('name') + '="false"';
					else if(!jQuery(this).hasClass('forced'))
		...
		foreach($shortcode['options'] as $option => $options) {
			...
			switch ($options['type']) {
				...
				case 'number':
					...
					break;
				case 'checkbox':
					$checked = (isset($options['default']) && $options['default']) ? ' checked="checked"' : '';
					$forced = (!empty($checked)) ? ' forced' : '';
					echo '<td class="label"><label for="'.$id.'_'.$option.'">'.$options['title'].'</label></td>';
					echo '<td class="field"><input type="checkbox"'.$checked.' id="'.$id.'_'.$option.'" class="sc_option'.$forced.'" name="'.$option.'" value="true" onClick="refreshCode();" /></td>';
					break;
				case 'color':
