...
function e404_slideshow_taxonomy_init() {
	$labels = array(
					'name' => __('Slideshows', 'natural'),
					'singular_name' => __('Slideshows', 'natural'),
					'all_items' => __('Slideshows', 'natural'),
					'edit_item' => __('Edit Slideshow', 'natural'),
					'update_item' => __('Update Slideshow', 'natural'),
					'add_new_item' => __('Add New Slideshow', 'natural'),
					'new_item_name' => __('New Slideshow Name', 'natural'),
					'search_items' => __('Search Slideshows', 'natural'),
					'parent_item' => __('Parent Slideshow', 'natural')
	);
...
function e404_slide_post_type_init() {
	$labels = array(
					'name' => __('Slides', 'natural'),
					'singular_name' => __('Slide', 'natural'),
					'add_new_item' => __('Add New Slide', 'natural'),
					'edit_item' => __('Edit Slide', 'natural'),
					'new_item' => __('New Slide', 'natural'),
					'view_item' => __('View Slide', 'natural'),
					'search_items' => __('Search Slides', 'natural'),
					'not_found' => __('No slides found', 'natural')
	);
...
function e404_custom_featured_image_text($content) {
	...
	if($post_type == 'e404_slide') {
		$content = str_replace(__('Set featured image'), __('Set slide image', 'natural'), $content);
		$content = str_replace(__('Remove featured image'), __('Remove slide image', 'natural'), $content);
	}
	return $content;
}
...
function e404_portfolio_post_type_init() {
	$labels = array(
					'name' => __('Portfolio', 'natural'),
					'singular_name' => __('Portfolio', 'natural'),
					'add_new_item' => __('Add New Portfolio Item', 'natural'),
					'edit_item' => __('Edit Portfolio Item', 'natural'),
					'new_item' => __('New Portfolio Item', 'natural'),
					'view_item' => __('View Portfolio Item', 'natural'),
					'search_items' => __('Search Portfolio', 'natural'),
					'not_found' => __('No portfolio items found', 'natural')
	);
...
function e404_get_taxonomy_name($taxonomy) {
	...
	switch ($taxonomy) {
		case 'categories':
			...
			if(!$name || empty($name))
				$name = __('Portfolio Categories', 'natural');
			break;
		case 'tags':
			$name = get_option('e404_portfolio_tags_name');
			if(!$name || empty($name))
				$name = __('Portfolio Tags', 'natural');
			break;	
	}
	return $name;
}
...
function e404_slideshow_columns($defaults) {
	$defaults['e404_slideshow'] = __('Slideshow', 'natural');
...
function e404_portfolio_columns($defaults) {
	...
	$defaults['portfolio-category'] = __('Category', 'natural');
	$defaults['portfolio-tag'] = __('Tags', 'natural');
	if(isset($defaults['e404_likes'])) {
		unset($defaults['e404_likes']);
	}
	if($e404_all_options['e404_portfolio_like_this'] == 'true') {
		$defaults['e404_likes'] = __('Likes', 'natural');
	}
	return $defaults;
}
