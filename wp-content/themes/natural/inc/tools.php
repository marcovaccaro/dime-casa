...
function e404_get_tweets($user, $count, $replies = false) {
	...
	if(($last_update < (time() - $e404_all_options['e404_twitter_expire'])) || !$text || !is_array($text)) {
		$params = array('screen_name' => $user, 'include_rts' => true);
		if($replies) {
			$params['count'] = $count;
		}
		else {
		...
		$url = "http://api.twitter.com/1/statuses/user_timeline.xml?".http_build_query($params);
		...
		if(is_object($xml)) {
			...
			foreach($xml->status as $status) {
				...
				$status_response['id'] = (int)$status->id;
...
function e404_breadcrumbs() {
	...
	echo '<a href="'.home_url().'">'.__('Home', 'natural').'</a>';
...
// subpages navigation
function e404_subpages_nav($in_widget = false) {
	global $post;
...
function e404_show_tags($tags) {
	if(empty($tags))
		return;
	$output = '';
	foreach($tags as $tag) {
		$output .= '<a href="'.get_tag_link($tag).'"><span>'.$tag->name.'</span></a> ';
	}
	$output = substr($output, 0, strlen($output) - 1);
	echo $output;
}

// get video (YouTube and Vimeo) ID from the video URL
function e404_get_video_id($url) {
	$site = '';
	...
	switch($site) {
		case 'youtube':
			preg_match("#(/v/|/watch\?v=)([A-Za-z0-9_-]{5,11})#", $url, $matches);
...
function e404_likes_text($likes) {
	if($likes == 1)
		return __('1 person likes it', 'natural');
	else
		return sprintf(__('%d people like it', 'natural'), $likes);
}
